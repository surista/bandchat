...
- **Audio playback**: `mobile/src/utils/audioPlaybackCoordinator.js` (`broadcastAudioPlay()` / `subscribeAudioPause()`) coordinates one-at-a-time playback across every audio player in the app — message bubbles, song attachments, the recordings list, a recording's detail screen. It used to be a Set private to `MessageBubble.js`, which meant only message-bubble players ever actually coordinated with each other; pulled out in the September 2026 review so the other three audio player components (which had never registered with it) could too. If you add a new audio player component, register it with this same module.
...
- **Deferred UI items** (low priority, captured here so they're not re-discovered). Verified against the tree as of the September 2026 full-app review — several entries from the July pass were already done, one list was found to be overbroad and trimmed to what's actually still true, and a new offender turned up — so trust this list over memory but re-check before starting:
  - **ActionSheet migration**: down to 5 genuine offenders that still hand-roll a bottom sheet with a custom `Modal` instead of the `<ActionSheet>` component (which routes to `ActionSheetIOS.showActionSheetWithOptions` for a native UIAlertController on iOS): `SettingsScreen` (workspace avatar action menu), `SongIntelligenceScreen`, `GigListScreen`, `WorkspaceListScreen`, `ChannelSettingsScreen`, plus `KittyScreen` (Type/Category pickers — newly found, wasn't on this list before). Migrate 1–2 per PR. Dropped from the July list: `SecurityScreen` (its modals are text-input forms, not option pickers — `ActionSheetIOS` can't replace those anyway) and `SetlistDetailScreen`/`SongDetailScreen`/`RecordingDetailScreen` (their remaining modals — song picker w/ search, performer multi-select, key picker, practice-log/edit-details forms — are all data-entry/multi-select surfaces, never straightforward 1:1 swaps; `SetlistDetailScreen` had in fact already migrated its MC-duration picker).
  ...
  - **Dynamic Type gap**: `maxFontSizeMultiplier` is applied thoroughly in some screens (`MessageBubble.js`, `SettingsScreen.js`, `SongListScreen.js`, `SetlistDetailScreen.js`, `LiveModeScreen.js`, `AppearanceScreen.js`...) but is entirely absent from roughly 20 others across every area of the app (e.g. `ReactionUsersSheet`, `PinnedMessagesScreen`, `SavedMessagesScreen`, `ActivityScreen`, `ChannelSettingsScreen`, `MemberProfileScreen`, `ShareReceiveScreen`, `EditProfileScreen`, `WebsiteSettingsScreen`, `NotificationsScreen`, `InviteScreen`, `UpgradeScreen`, `WorkspaceMembersScreen`, `BlockedUsersScreen`, `SongDetailScreen`, `SongIntelligenceScreen`, `MedleyListScreen`, `MedleyDetailScreen`, `RecordingListScreen`, `RecordingDetailScreen`, `PracticeDashboardScreen`). Found in the September 2026 review; deliberately not bulk-fixed in that pass (mechanical but large — hundreds of individual prop additions with no device available to visually verify) — worth doing as its own dedicated sweep.
  - **Stage plot coordinate space**: mobile's `StagePlotEditorScreen.js` stores item x/y relative to the live editor's on-screen canvas (measured via `onLayout`, so it varies per device), but persists a hardcoded `stageWidth: 900, stageHeight: 500` and the PDF export used to place items using those same raw device-relative coordinates on a fixed 900x500 print canvas with no scaling (fixed in the September 2026 review — export now scales by `900/stageLayout.width` etc). The web editor (`client/src/components/band/StagePlotCreator.jsx`) treats that 900x500 as a real, literal, device-independent canvas. Net effect: a plot built on mobile likely still displays misplaced if opened on web (or on a different mobile device), since its coordinates were never actually normalized to the nominal 900x500 space at save time. Proper fix needs the mobile editor to render at a fixed logical size (scaled to fit the screen) like web does, or to convert coordinates to/from the 900x500 space on load/save — bigger surgery than the print-export fix, deliberately not attempted without a device to verify drag behavior against.
  - Done since the July pass (do not re-file): `accessibilityViewIsModal` (`WorkspaceListScreen.js`'s Join Workspace modal was the last holdout), per-screen `ErrorBoundary` (now via the `layout` prop in `AppStack.js`), `Switch trackColor`, `KeyboardAvoidingView behavior` on Android, and the `keyboardDismissMode` sweep for ScrollViews containing text input.
...
- **Refresh tokens are single-use and rotate with no grace period.** `POST /api/auth/refresh` deletes the presented token and issues a new one, so two refreshes racing on the same token means the loser gets `401 "Refresh token has been revoked"` — which both clients treat as a definitive auth failure (`clearTokens()`, then bounce to login). Deduplication therefore lives **inside** `refreshAccessToken()` on both platforms (`client/src/services/api.js`, `mobile/src/services/api.js`), not in the callers — never call the underlying refresh directly, and never "helpfully" add a caller-side lock instead. This caused the v1.07.39 web login loop (the socket's `connect_error` handler re-fires on every reconnect attempt and was launching a burst of unsynchronized refreshes) and, earlier, mobile logouts on app resume.
  - **Still open:** dedup was per-client-instance, so two browser tabs could still race — whichever tab refreshed first invalidated the other's stored token, and that tab's *next* refresh (even a plain reload) came back "revoked" and forced a full re-login even though the session was fine. v1.07.52 mitigated the common case for web: `ApiService` now broadcasts every `setTokens()`/`clearTokens()` over a `BroadcastChannel('bandchat-auth')` so same-origin tabs stay in sync in real time (see the constructor comment and `_applyAuthBroadcast()` in `client/src/services/api.js`). Web ↔ mobile can still race (no channel between platforms), and a tab that was closed and reopened *after* other tabs already rotated the token several times in the meantime is still out of luck — broadcasts only reach tabs that are open to receive them. The full fix for those remains a short server-side reuse-grace window on rotation, which needs a schema change. Worth doing if logout-loop reports resurface.
...
- **Every emoji-selection surface leads with the user's most-used emojis.** Ranking lives in `client/src/utils/emojiFrequency.js` and `mobile/src/utils/emojiFrequency.js` — mirrored implementations sharing the `emojiFrequency` storage key and shape (`{ [emoji]: { n: count, t: lastUsedMs } }`, ranked by count then recency, counts halved at 200 so old habits decay, map capped at 60). `getFrequentEmojis(limit)` always returns `limit` entries, padding a thin history with `DEFAULT_FREQUENT` so rows never render short or resize as they load. Mobile is async, so pair it with the synchronous `peekFrequentEmojis(limit)` for the first frame; web is synchronous. **Any new code path that applies an emoji must call `trackEmojiUsage(emoji)`** — including ones that bypass a picker (mobile swipe-to-react, the `MessageActionSheet` quick row), which is exactly what was missing before. Mobile seeds itself once from the legacy `recentEmojis` MRU list in `services/storage.js`; that list is no longer written to.
