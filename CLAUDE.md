...
- **Deferred from the June 2026 code-review pass** (low priority, captured here so they're not re-discovered):
  - **`accessibilityViewIsModal` sweep** — done for 5 highest-volume Modals (ActionSheet, MessageActionSheet, EmojiPicker, WorkspaceSwitcher, ReactionUsersSheet). Remaining ~20 screen-level `<Modal>` instances still lack it. Pattern: add `accessibilityViewIsModal` to the inner sheet `Pressable`/`View` (not the outer overlay).
  - **30-screen ActionSheet migration**: ~30 screens reimplement bottom action sheets with custom `Modal` + JS sheet instead of using the existing `<ActionSheet>` component (which routes to `ActionSheetIOS.showActionSheetWithOptions` for native UIAlertController on iOS). Migrate gradually — 1-2 screens per PR. Affected: `GigListScreen`, `BandMembersScreen`, `MedleyListScreen`, `RecordingListScreen`, `SongListScreen`, `SetlistListScreen`, `PollsScreen`, `KittyScreen`, `WorkspaceMembersScreen`, `ChannelListScreen`, `TimelineScreen`, others.
  - **Per-screen ErrorBoundary**: only `App.js` has one. A render error in any screen blanks the whole app. Add a thin boundary inside RootNavigator or per-stack.
  - **Switch trackColor**: fixed in `NotificationsScreen.js`. Same pattern (`Platform.OS === 'ios' ? undefined : { ... }`) should be applied to `AppearanceScreen.js`, `WebsiteSettingsScreen.js`, `SecurityScreen.js`, `ChannelSettingsScreen.js`, `OnboardingWizardScreen.js` so iOS gets the system green track.
  - **ScrollView `keyboardDismissMode`**: ~10 ScrollView screens missing `keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'}`.
  - **KeyboardAvoidingView `behavior="height"` on Android**: with `softwareKeyboardLayoutMode: 'resize'` already set in app.config.js, the OS resizes the window; the JS behavior=height then double-adjusts. Fix is `behavior={Platform.OS === 'ios' ? 'padding' : undefined}` across `SetlistDetailScreen`, `LoginScreen`, `KittyScreen`, others.
  - **Typed `Message` shape**: the Message object passes through Socket / SQLite / UI / api.js with implicit shape. The recent v1.07.06/07 thread-reply count fixes and v1.07.18 dedupe fixes all clustered around this surface. If introducing TypeScript incrementally, convert `Message` first.
