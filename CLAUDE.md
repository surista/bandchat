...
- **Known TODO — `React.memo` extraction on MessageList rows:** The row JSX (~410 lines, ~30 closure dependencies including `editingId`, `editContent`, `editTextareaRef`, edit toolbar handlers, multiple modal triggers, `pinnedMessageIds`/`savedMessageIds`/`memberAvatarMap` Sets/Maps) currently re-renders every row on every socket event because there's no memoization boundary between the parent and each row. Single emoji reaction by anyone in a 5,000-message channel = 5,000-row re-render storm (~100-200ms). Fix is a focused PR: extract `MessageRow` at module scope, accept per-row props as primitives (`isPinned: boolean`, `isSaved: boolean`, `isEditing: boolean`, `isHighlighted: boolean`, `isReactionPickerOpen: boolean`) instead of the parent's Sets/state, wrap with `React.memo` with default shallow comparison. Socket handlers in `ChannelView` already use `prev.map(m => m.id === id ? {...m, ...} : m)` so unchanged messages keep their object reference — `React.memo`'s shallow compare will short-circuit them. Bundle helpers/handlers into a `rowCtx` object with `useMemo` at the parent so context changes don't bust memoization. **Don't ship this alongside other refactors** — needs manual exercise of every interaction (long-press, hover, edit, delete, reactions, pin, save, jump-to-message).
