      ...
      <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={onClose} accessibilityRole="button" accessibilityLabel="Dismiss action sheet">
        <View style={[styles.sheet, { backgroundColor: colors.modalBg }]}>
          ...
          {actions.map((action, i) => (
            <TouchableOpacity key={i} style={styles.actionItem} onPress={action.onPress} accessibilityRole="button" accessibilityLabel={action.label}>
              <Text style={[styles.actionText, { color: action.destructive ? '#ef4444' : colors.textPrimary }]}>{action.label}</Text>
            </TouchableOpacity>
          ))}
          <TouchableOpacity style={[styles.actionItem, styles.cancelItem]} onPress={onClose} accessibilityRole="button" accessibilityLabel="Cancel">
