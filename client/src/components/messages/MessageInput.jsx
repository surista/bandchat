        ...
        <div className="flex items-center justify-between px-3 py-2 border-t border-[var(--color-border)]">
          <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-0.5 pr-2 mr-2 border-r border-[var(--color-border)]">
              <button type="button" onClick={() => wrapSelection('**')} className="p-1.5 text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors rounded hover:bg-[var(--color-bg-tertiary)]" aria-label="Bold" title="Bold (Ctrl+B)" disabled={sending || isRecording}>
                <span className="font-bold text-sm w-5 h-5 flex items-center justify-center">B</span>
              </button>
              <button type="button" onClick={() => wrapSelection('*')} className="p-1.5 text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors rounded hover:bg-[var(--color-bg-tertiary)]" aria-label="Italic" title="Italic (Ctrl+I)" disabled={sending || isRecording}>
                <span className="italic text-sm w-5 h-5 flex items-center justify-center">I</span>
              </button>
              <button type="button" onClick={() => wrapSelection('~~')} className="p-1.5 text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors rounded hover:bg-[var(--color-bg-tertiary)]" aria-label="Strikethrough" title="Strikethrough (Ctrl+Shift+X)" disabled={sending || isRecording}>
                <span className="line-through text-sm w-5 h-5 flex items-center justify-center">S</span>
              </button>
              <button type="button" onClick={() => wrapSelection('`')} className="p-1.5 text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors rounded hover:bg-[var(--color-bg-tertiary)]" aria-label="Inline code" title="Code (Ctrl+E)" disabled={sending || isRecording}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>
              </button>
              <button type="button" onClick={() => wrapSelection('```\n', '\n```')} className="p-1.5 text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors rounded hover:bg-[var(--color-bg-tertiary)]" aria-label="Code block" title="Code block" disabled={sending || isRecording}>
              ...
              <button type="button" onClick={() => { const ta = textareaRef.current; if (ta) { const start = ta.selectionStart; const lineStart = content.lastIndexOf('\n', start - 1) + 1; setContent(content.slice(0, lineStart) + '- ' + content.slice(lineStart)); setTimeout(() => { ta.focus(); ta.setSelectionRange(start + 2, start + 2); }, 0); }}} className="p-1.5 text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors rounded hover:bg-[var(--color-bg-tertiary)]" aria-label="Bullet list" title="Bullet list" disabled={sending || isRecording}>
