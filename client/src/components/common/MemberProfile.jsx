...
export default function MemberProfile({ userId, workspaceId, onClose, onStartDM }) {
  ...
  return (
    <Modal isOpen={true} onClose={onClose} maxWidth="max-w-md">
      {loading ? (
        <div className="p-8 text-center text-[var(--color-text-muted)]">Loading profile...</div>
      ) : error ? (
        ...
        <>
            ...
            <div className="absolute -bottom-12 left-6">
              {profile.user.avatarUrl ? (
                <img
                  ...
                  className="w-24 h-24 rounded-xl border-4 border-[var(--color-bg-primary)] object-cover"
                />
              ) : (
                <div className="w-24 h-24 rounded-xl border-4 border-[var(--color-bg-primary)] bg-blue-600 flex items-center justify-center text-white text-3xl font-bold">
          ...
          <div className="pt-14 px-6 pb-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h2 className="text-xl font-bold text-[var(--color-text-primary)]">{profile.user.displayName}</h2>
                <button
                  type="button"
                  className="text-[var(--color-text-muted)] text-sm select-all cursor-pointer hover:text-[var(--color-text-secondary)] bg-transparent border-none p-0 text-left"
              ...
              <span className={`px-2 py-1 rounded text-xs ${
                profile.role === 'ADMIN' ? 'bg-yellow-600 text-yellow-100' : 'bg-[var(--color-bg-tertiary)] text-[var(--color-text-secondary)]'
              }`}>
                {profile.role}
              </span>
            </div>
            ...
            {profile.user.bio && (
              <p className="text-[var(--color-text-secondary)] mb-4">{profile.user.bio}</p>
            )}
            ...
            <div className="grid grid-cols-4 gap-2 mb-6">
              <button
                ...
                className={`bg-[var(--color-bg-tertiary)]/50 rounded-lg p-3 text-center transition-colors ${
                  profile.stats.gigsAttended > 0 ? 'hover:bg-[var(--color-bg-hover)] cursor-pointer' : ''
                }`}
              >
                <p className="text-xl font-bold text-[var(--color-text-primary)]">{profile.stats.gigsAttended || 0}</p>
                <p className="text-xs text-[var(--color-text-muted)]">Gigs</p>
              </button>
              <button
                onClick={() => profile.stats.rehearsalsAttended > 0 && loadEvents('REHEARSAL')}
                className={`bg-[var(--color-bg-tertiary)]/50 rounded-lg p-3 text-center transition-colors ${
                  profile.stats.rehearsalsAttended > 0 ? 'hover:bg-[var(--color-bg-hover)] cursor-pointer' : ''
                }`}
              >
                <p className="text-xl font-bold text-[var(--color-text-primary)]">{profile.stats.rehearsalsAttended || 0}</p>
                <p className="text-xs text-[var(--color-text-muted)]">Rehearsals</p>
              </button>
              <div className="bg-[var(--color-bg-tertiary)]/50 rounded-lg p-3 text-center">
                <p className="text-xl font-bold text-[var(--color-text-primary)]">{profile.achievements?.length || 0}</p>
                <p className="text-xs text-[var(--color-text-muted)]">Badges</p>
              </div>
              <div className="bg-[var(--color-bg-tertiary)]/50 rounded-lg p-3 text-center">
                <p className="text-xl font-bold text-[var(--color-text-primary)]">{profile.stats.messages}</p>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {profile.achievements.map(achievement => (
                    <div
                      key={achievement.id}
                      className="flex items-center gap-1.5 bg-[var(--color-bg-tertiary)] rounded-full px-3 py-1.5"
                      title={`${achievement.name}: ${achievement.description}`}
                    >
                      <span className="text-lg">{achievement.icon}</span>
                      <span className="text-sm text-[var(--color-text-primary)]">{achievement.name}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {profile.achievements.length === 0 && (
              <div className="mb-6 text-center py-4 bg-[var(--color-bg-tertiary)]/30 rounded-lg">
                <p className="text-[var(--color-text-muted)] text-sm">No badges earned yet</p>
              </div>
            )}
      ...
      {eventsModal && (
        ...
        >
          <div
            className="bg-[var(--color-bg-secondary)] rounded-lg shadow-xl max-w-sm w-full max-h-[70%] flex flex-col border border-[var(--color-border)]"
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-3">
              {eventsModal.loading ? (
                <div className="text-center text-[var(--color-text-muted)] py-6">Loading...</div>
              ) : eventsModal.error ? (
                <div className="text-center text-red-400 py-6">{eventsModal.error}</div>
              ) : eventsModal.events.length === 0 ? (
                <div className="text-center text-[var(--color-text-muted)] py-6">No events found</div>
              ) : (
                <div className="space-y-2">
                  {eventsModal.events.map(event => (
                    <div
                      key={event.id}
                      className="bg-[var(--color-bg-tertiary)]/50 rounded-lg p-2.5"
                    >
                      <div className="font-medium text-[var(--color-text-primary)] text-sm">{event.title}</div>
